-- The scope-inheritance step used min(department_id) on a uuid column, and
-- PostgreSQL ships no min() aggregate for uuid:
--   ERROR 42883: function min(uuid) does not exist
-- Generation failed outright for every caller. Take the single distinct value
-- out of array_agg instead, which needs no aggregate for the type.
--
-- This is the current definition of generate_schedule: it enforces supervisor
-- scoping itself (SECURITY DEFINER writes past RLS), skips templates outside the
-- caller's departments and reports them in `skipped_templates`, and stamps the
-- new schedule with the department/location its templates agree on so a scoped
-- supervisor can still publish what they generated.
...
begin
  ...
  select coalesce(array_agg(t.name), '{}')
    into skipped
  from _rules_ok x
  join public.shift_templates t on t.id = x.template_id
  where not public.can_manage_scope(caller, cid, x.department_id, x.location_id);

  delete from _rules_ok x
   where not public.can_manage_scope(caller, cid, x.department_id, x.location_id);

  if not exists (select 1 from _rules_ok) then
    raise exception 'none of the selected templates are within your assigned departments';
  end if;
  ...
  if not public.can_manage_scope(caller, cid, sched_dept, sched_loc) then
    raise exception 'the selected templates span departments outside your assignment; generate them separately';
  end if;
  ...
  while d <= _ends_on loop
    ...
    for rule in select x.rule from _rules_ok x loop
      ...
      if tpl.end_time > tpl.start_time then
        ...
        -- Overnight template: ends the following day. Availability is only
      ...
      for i in 1..headcount loop
        ...
        order by
          -- required position first, then whoever has the lightest week so far
          case
        ...
        if chosen is null then
          -- Nobody eligible: keep the slot as an open shift so the gap is visible.
          open_cnt := open_cnt + 1;
        else
          assigned := assigned + 1;
          insert into _load (employee_id, week_start, hours)
