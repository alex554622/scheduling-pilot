...
import { Wallet, Check, EyeOff, Trash2 } from "lucide-react";
...
export function PaySettingsCard({
  ...
  function save(e: React.FormEvent) {
    ...
    for (const [label, value] of [["Federal tax", taxPercent], ["State tax", statePercent]] as const) {
      ...
      <form className="space-y-3" onSubmit={save}>
        ...
        <div className="flex items-center gap-2">
          <Button type="submit" size="sm">
            {saved ? <><Check className="mr-1.5 h-3.5 w-3.5" />Saved</> : "Save"}
          </Button>
          {settings && (
